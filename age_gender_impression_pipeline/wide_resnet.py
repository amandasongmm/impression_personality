...
#         self._k = k
#         self._dropout_probability = 0
#         self._weight_decay = .0005
#         self._use_bias = False
#         self._weight_init = "he_normal"
#
#         if K.image_dim_ordering() == "th":
#             logging.debug('image-dim-ordering="th"')
#             self._channel_axis = 1
#             self._input_shape = (3, image_size, image_size)
#         else:
#             logging.debug('image-dim-ordering="tf"')
#             self._channel_axis = -1
#             self._input_shape = (image_size, image_size, 3)
#
#     # Wide Residual network: http://arxiv.org/abs/1605.07146
...
#                         convs = Dropout(self._dropout_probability)(convs)
#                     convs = Conv2D(n_bottleneck_plane, kernel_size=(v[0], v[1]),
#                                    strides=v[2], padding=v[3], kernel_initializer=self._weight_init,
#                                    kernel_regularizer=l2(self._weight_decay),
#                                    use_bias=self._use_bias)(convs)
#
#             # Shortcut Connection: identity function or 1x1 convolutional
...
#             #   group; see _layer() ).
#             if n_input_plane != n_output_plane:
#                 short_cut = Conv2D(n_output_plane, kernel_size=(1, 1),
#                                    strides=stride, padding="same", kernel_initializer=self._weight_init,
#                                    kernel_regularizer=l2(self._weight_decay), use_bias=self._use_bias)(net)
#             else:
#                 short_cut = net
#
#             return add([convs, short_cut])
#
#         return f
#
#     # "Stacking Residual Units on the same stage"
...
#         # Add wide residual blocks
#         block_fn = self._wide_basic
#         conv2 = self._layer(block_fn, n_input_plane=n_stages[1], n_output_plane=n_stages[1], count=n, stride=(1, 1))(conv1)
#         conv3 = self._layer(block_fn, n_input_plane=n_stages[1], n_output_plane=n_stages[2], count=n, stride=(2, 2))(conv2)
#         conv4 = self._layer(block_fn, n_input_plane=n_stages[2], n_output_plane=n_stages[3], count=n, stride=(2, 2))(conv3)
#         batch_norm = BatchNormalization(axis=self._channel_axis)(conv4)
#         relu = Activation('relu')(batch_norm)
